...
moreFiles=[(id_generator() + ".txt", random.randint(10, 100000)) for i in range(100)]
moreNodes=[("node{}".format(i+3), random.randint(10,1000000)) for i in range(40)]
files_df = files_df.append(pd.DataFrame.from_records(moreFiles, columns=("filename","size")))
nodes_df = nodes_df.append(pd.DataFrame.from_records(moreNodes, columns=("nodes", "available_space")))


## Simply run the main algorithm again with more data
